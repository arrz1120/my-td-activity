...
export default class Rule extends Component{
    ...
    buildRuleBtn(){
        $("body").append('<div class="rule-btn">活动<br />规则</div>');
        $(".rule-btn").on("click",(e)=>{
            if(window.sa){
    ...
    setIdx(i){
        ...
        this.ruleSwiper.slideTo(i,0,false);
    }
    initSwiper(){
        ...
        let ruleTxtSwiper1 = new Swiper('.ruletxt-swiper1', {
            ...
            scrollbarHide:false,
            onTouchEnd:(swiper)=>{
                let {realIndex,translate,isEnd}=swiper;
                if(realIndex===0&&translate>50){
                    this.ruleSwiper.slideTo(0);
                }
                if(isEnd){
                    this.ruleSwiper.slideTo(2);
                }
              }
         });
    ...
    render(props,state){
        ...
        return(
            <div class="mask" style={{display:!state.isShow?'none':''}}>
                <div
                    onClick={()=>{
                        this.setState({
                            isShow:false
                        ...
                        <div class="rule-nav">
                            {
                                navItems.map((item,i)=>{
                                    return (
                                        <span
                                            // dangerouslySetInnerHTML={item.val}
                                            key={i}
                                            onClick={(e)=>{
                                                this.setState({
                                                    index:i
                        ...
                        <div class="swiper-container rule-swiper">
                            <div class="swiper-wrapper">
                                <div class="swiper-slide">
                                    <ul class="txt1">
                                        <li>
                                            <span>①</span>活动期间，用户成功加入（且匹配成功）We自动服务、We+自动服务（新手专享服务除外）的金额均可享受0.8%的加息奖励；
                                        </li>
                                        <li>
                                            <span>②</span>若活动期间开放的We自动服务、We+自动服务未能在活动期内满额，则该加息可延续至其满额为止。
                                        </li>
                                    </ul>
                                </div>
                                <div class="swiper-slide">
                                    <div class="swiper-container ruletxt-swiper ruletxt-swiper1">
                                        <div class="swiper-wrapper">
                                            <div class="swiper-slide">
                                                <ul class="txt2">
                                                    <li>
                                                        <span>①</span>登录团贷网APP并进入活动页面即可获得1个粽子，每天可获得1个；
                                                    </li>
                                                    <li>
                                                    ...
                                                    <li>
                                                        <span>④</span>用户累计加入金额（智享转让及债权转让除外）满1000元即可获得1个粽子，最多可获得50个；
                                                    </li>
                                                    <li>
                                                        <span>⑤</span>兑换的红包于每天0点限量放出，所兑换的红包使用规则请参考团宝箱；
                                                    </li>
                                                    <li>
                                                        <span>⑥</span>活动期间所获得的粽子为虚拟物品，仅限本次活动中兑换红包及抵扣助力商品使用，活动结束后剩余的粽子数将失效。
                                                    </li>
                                                </ul>
                                            </div>
                                        </div>
                                        ...
                                        <div class="swiper-wrapper">
                                            <div class="swiper-slide">
                                                <ul class="txt2">
                                                    ...
                                                    <li>
                                                        <span>⑤</span>获得奖品将于活动结束后15个工作日内发放，请注意到团宝箱-精美礼品中查收，过期未填写地址，视为放弃，不予补发且不支持退换货、兑现、不提供发票等；
                                                    </li>
                                                    <li>
