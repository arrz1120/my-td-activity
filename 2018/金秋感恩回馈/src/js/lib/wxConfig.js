 ...
 *配置参考网址： http://203.195.235.76/jssdk/#menu-basic
*/
...
let wxData = {
    ...
    jsticketurl: 'https://m.tuandai.com',
...
export function WeiXinShare(shareUrl, shareTitle, shareDesc, shareImgUrl, shareSuccessCallback) {
    wxData.url = shareUrl;
    wxData.title = shareTitle;
    wxData.desc = shareDesc;
    wxData.img_url = shareImgUrl;
    wxData.ShareCallBack = shareSuccessCallback;
    init();
}
...
function init() {
    ...
    if (wxData.isWxJsSDK) {
        ...
        // let url = wxData.jsticketurl + "/ajaxCross/WXTokenAjax.ashx"
        // let data = {code: wxcode, url: wxData.csrf_url, r: Math.random()}
        // jsonp(url,data).then(res => {
        //     WeChatSDKjsonpCallback(res)
        // }).catch(err => {
        //     console.log(err)
        // })
    } else {
