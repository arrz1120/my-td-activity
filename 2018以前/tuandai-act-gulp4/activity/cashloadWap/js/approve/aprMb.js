(function() {
    ...
    function showResult(result) {
        ...
        var stepEl = $('.step2');
        if (result === '1') {
            resultInfo = '<i class="icon-inline icon-success"></i> 您已完成手机运营商认证！';
            resultBtnsTemp = '<div class="btn btn-full" id="btn_apr_ok">确定</div><a href="#" class="link-yellow btn-restart">重新认证</a>';
        } else {
            resultInfo = '<i class="icon-inline icon-fail"></i> 认证失败，请重试！';
        ...
        hideLoading();
        stepEl.removeClass('hidden');
        // var _info = result == 1 ? '您已完成手机运营商认证！': '认证失败，请重试！',
        //     _btn_name = result == 1 ? '确定': '马上重试'
        // Util.showResult({
        //     result: 1,
        //     status: '',
        //     info: _info,
