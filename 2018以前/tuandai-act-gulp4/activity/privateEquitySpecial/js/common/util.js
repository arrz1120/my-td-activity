(function(global) {
    ...
    */
    function popup(options) {
        var _options = {
            "msg": "",
            "type": 1, //1--电话呼叫弹窗, 2--自定义弹窗
            "alertConClassName": "order-con",
            "title": "在线预约咨询",
            "content": "",
            "btns": [{
                "txt": "",
        ...
        switch(_options.type) {
            case 1:
                var alert_con = alert_con.addClass("alert-con"),
            ...
            case 2:
                ...
                alert_con = alert_con.addClass(_options.alertConClassName).append(_options.content);
                var alert_btns = $("<div/>").addClass("alert-btns");
                if (_options.btns.length > 1) {
                    var cancel_btn = $("<span/>").addClass("alert-btn " + _options.btns[0].className).html(_options.btns[0].txt),
                        confirm_btn = $("<span/>").addClass("alert-btn " + _options.btns[1].className).html(_options.btns[1].txt);
                    cancel_cb = _options.btns[0].cb;
                    confirm_cb = _options.btns[1].cb;
                    alert_btns.append(cancel_btn).append(confirm_btn);
                }else{
                    confirm_btn = $("<span/>").addClass("alert-btn " + _options.btns[0].className).html(_options.btns[0].txt);
                    confirm_cb = _options.btns[0].cb;
                    alert_btns.append(confirm_btn);
                }
                alert_con.append(alert_btns);
                break;
        }
