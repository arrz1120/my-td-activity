...
function OnClickMessage()
{
    $.ajax({
        async: false,
        url: "/201706/SignIn/ajax/reg.ashx",
        dataType: "json",
        data: {
...
{
    if (checkagree() && BlurTelNo(true) && BlurPwd(true) && BlurImgCode(true) && BlurCode(true))
    {
        $(ctrl).unbind("click");
        var vOldBtnRegText = $("#btnReg").html();
        $("#btnReg").html("正在提交注册信息...");

        var from = "activity_xianshituijian_20160401";
        ...
        $.ajax({
            ...
            data: {
                ...
                extendKey: extenderKey
            },
            success: function (returnJsonData)
            {
                if (returnJsonData.result == 1)
                {
                    window.location.href = Globals.MobileWebsiteUrl + '/pages/downopenapp.aspx?type=formreg'
                }
                else
                {
            ...
            success: function (returnJsonData)
            {
                ...
                else
                {
                    switch (result)
                    {
                        ...
                        case "-6":
                            $("#dvCodeError").html(returnJsonData.message);
                            reloadCode('/UserRegister/ajaxhander/ValidateCode.ashx')
