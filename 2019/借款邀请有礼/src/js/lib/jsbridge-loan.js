...
;(function(win) {
    ...
    function exec(methodName, cb, params) {
        params = params || {};
        isIOS() ? iosHandler(methodName, params, cb) : androidHandler(methodName, params, cb);
    }
    /**
     * @func
    ...
    JsBridge.prototype = {
        ...
        gotoCertificate: function() {
            this.exec('gotoCertificate');
        },
        gotoLoan: function() {
            this.exec('gotoLoan');
        },
        gotoShare: function(cb, param) {
            console.log(param)
            this.exec('gotoShare', cb, param);
        },
        gotoCouponPage: function() {
            this.exec('gotoCouponPage');
        },
        closeWeb: function() {
            this.exec('CloseWeb');
        },
        toAppCallBack: function(reply, cb) {
            this.exec('toAppCallBack', cb, reply);
        },
        toAppExtend: function(params) {
            var methodName = 'ToAppExtend';
            if (isIOS()) {
                methodName = 'ToAppIosExtend';
            }
            this.exec(methodName, null, params);
        },
