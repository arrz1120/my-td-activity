...
<body class="bg-f1f3f5">
    ...
    <div class="z-top">
		<div class="pro-top bb-e6e6e6 bg-fff">
		    <% if (GlobalUtils.IsShowZxToInvestor(WebUserAuth.UserId.Value) || new GlobalUtils().GetNewVipUserInfo(WebUserAuth.UserId.Value).Level >= 3)
		       {
		    %>
            <div class="pro-top-item w33p f17px text-center top-item-cur" id="WePlan">自动投标服务</div>
			<div class="pro-top-item w33p f17px text-center" id="ZxPlan">智享转让服务</div>
			<div class="pro-top-item w33p f17px text-center" id="Disperse">优质项目</div>
            ...
            <%
		       ...
		       {
		           %>
            <div class="pro-top-item w50p f17px text-center top-item-cur" id="WePlan">自动投标服务</div>
			<div class="pro-top-item w50p f17px text-center" id="Disperse">优质项目</div>
            <div class="pro-top-line" style="left:50%;"></div>
            <%
		       } %>
		</div>
		<div class="clearfix bg-fff pl20 pr20 pt6 pb6 pos-r z-index100">
			<div class="lf w50p text-center f16px c-ffcf1f" id="orderbyTime">投资时间
    ...
    <div id="wrapper" class="pb20" style="top: 110px;">
        <div id="scroller">
            ...
            <div class="pb20">
                <div class="record_list" id="thelist">
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $('#Disperse').bind("click", function () {
                ...
                $('#WePlan').removeClass("top-item-cur");
                $('#ZxPlan').removeClass("top-item-cur");
                $(this).addClass("top-item-cur");
            });

            $('#WePlan').bind("click", function () {
                TabClick("#WePlan", "", "WePlan");
                $('#Disperse').removeClass("top-item-cur");
                $('#ZxPlan').removeClass("top-item-cur");
                $(this).addClass("top-item-cur");
            });
            $('#ZxPlan').bind("click", function () {
                TabClick("#ZxPlan", "", "ZxPlan");
                $('#Disperse').removeClass("top-item-cur");
                $('#WePlan').removeClass("top-item-cur");
                $(this).addClass("top-item-cur");
            });
                ...
                success: function (jsonstr) {
                    ...
                    if (jsonstr.result == "1") {
                        ...
                        for (var i = 0; i < jsonstr.list.length; i++) {
                            if (jsonstr.list[i].Type.indexOf("We计划") != -1 || jsonstr.list[i].Type.indexOf("We自动服务") != -1) {
                                var weStatusHtml = "";
                                switch (jsonstr.list[i].WeStatus) {
                                    case "0":
                                        weStatusHtml = GetStatusHtml("c-ffbb45", "项目匹配中");
                                        break;
                                    default:
                                        weStatusHtml = GetStatusHtml("c-ffbb45", "投标￥" + jsonstr.list[i].Amount);
                                        break;
                                }
                            ...
                            } else if (jsonstr.list[i].Type.indexOf("复投宝") != -1 || jsonstr.list[i].Type.indexOf("速盈宝") != -1 || jsonstr.list[i].Type.indexOf("We+自动服务") != -1) {
                                var weStatusHtml = "";
                                switch (jsonstr.list[i].WeStatus) {
                                    case "0":
                                        weStatusHtml = GetStatusHtml("c-ffbb45", "项目匹配中");
                                ...
                                if (jsonstr.list[i].Status == "逾期") {
                                    ...
                                    weStatusHtml = GetStatusHtml("c-ffbb45", jsonstr.list[i].PreCycDate + " 回款(" + jsonstr.list[i].RefundedMonths + "/" + jsonstr.list[i].TotalRefundMonths + ")");
                                }
                                else if (jsonstr.list[i].Status == "持有中") {
                                    weStatusHtml = GetStatusHtml("c-ffbb45", "持有中");
                                }
                                else if (jsonstr.list[i].Status == "已赎回") {
                                    weStatusHtml = GetStatusHtml("c-ababab", "持有中");
                                }
                                var url = "/Member/Repayment/my_return_details.aspx";
                                if (searchType == "ZxPlan" && "<%=GlobalUtils.IsOpenSubscribeApi%>" =="False") {
                                    url = "//m.tdw.cn" + url;
                                }
        ...
        function GetReturnHtml(projectUrl, invest, status, statusHtml) {
            ...
            var html ='<div class="record_item bt-e6e6e6 bb-e6e6e6 mb10 bg-fff" onclick="javasript:window.location.href=\''+projectUrl+'\'">'+
				'<div class="clearfix">'+
					'<div class="w50p lf">'+
						'<p class="f17px text-overflow">' + invest.Title + '</p>' +
					'</div>'+
					'<div class="w50p rf text-right">'+
						'<p class="f17px">￥' + ((invest.Type.indexOf("We计划") != -1 || invest.Type.indexOf("复投宝") != -1) ? invest.PurchaseAmount : invest.Amount) + '</p>' +
					'</div>'+
				'</div>'+
				'<div class="clearfix bt-e6e6e6">'+
					'<div class="w50p lf">'+
						'<p class="f13px c-ababab">' + invest.SubscribeDate + " " + invest.TenderMode + '</p>' +
					'</div>'+
					'<div class="w50p rf text-right">'+
						statusHtml+
					'</div>'+
