    ...
    <div class="bb-e6e6e6 pl15 pr15 pt13 pb13 bg-fff"> 
	 ...
	 <div class="bg-fff bt-e6e6e6">
        ...
        <div class="pd15 bb-e6e6e6 webkit-box">
            ...
            <a class="box-flex1 f15px c-fab600 text-right text-overflow" href="<%=TuanDai.WXApiWeb.GlobalUtils.ContractViewUrl%>/p2p/weixin/contractZqzr.aspx?key=<%=item.ContractNo %>">查看合同</a>
		</div>
        <%  }
          }else{ %>
       <div class="pd15 bb-e6e6e6 webkit-box">
       ...
       function sendMobileValidSMSCode() {    
           ...
           jQuery.ajax({
               ...
               success: function (json) {
                   ...
                   else if (parseInt(result) == -99) { window.location.href = "//passport.tuandai.com/2login?ret=" + encodeURI(window.location.href); }
                   if (result == "1") {
                       leftsecond = 60; 
                       clearInterval(timer);
