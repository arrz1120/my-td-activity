<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="contractType24.aspx.cs" Inherits="TuanDai.WXApiWeb.Contract.contractType24" %>
...
<div class="Contract">
    <h3>借款合同</h3>
    <div class="Number">编号：<%=this.key %></div>
    <div class="Contract_t">
     ...
     <div class="Contract_c">
        ...
        <p>
                一、借款条款<br />
            1、借款金额：乙方因资金周转需要，现向甲方借款￥<span><%=this.submodel.Amount.Value.ToString("N")%></span>元（大写人民币：<span><%=ChineseNum.GetUpperMoney(double.Parse(this.submodel.Amount.Value.ToString()))%></span>）<br />
            ...
            5、提前还款：本次借款期间，乙方有权提前一次性结清本次债务。若乙方选择等额本息的还款方式，利息应按照实际借款期间足月计算(即不足一个月的按一个月计算)。 若乙方选择每月付息且到期本金的还款方式，在乙方提前结清本次借款时侯，应向甲方支付实际已产生的利息（即按照实际用款天数折比365个自然日计算）。
        </p>
        <p>
            二、保证担保<br />
...
<div class="Contract">
    ...
    <div class="Contract_b">
        ...
        <p>乙方：<span><%=itemProjectInfo.RealName%></span></p>
        <p>丙方：<span><%=this.assureModel != null?this.assureModel.FullName:""%></span></p>
        <p>协议签订地：<span>广东省东莞市南城区</span></p>
        <p>协议签署日期：<em><%=this.submodel.AddDate.Value.Year.ToString() %></em>年<em><%=this.submodel.AddDate.Value.Month.ToString() %></em>月<em><%=this.submodel.AddDate.Value.Day.ToString() %></em>日</p>
