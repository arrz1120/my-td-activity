...
<html>
	...
	<script type="text/javascript">
	    ...
	    function hideshow() {
			var tab = $(".mx-con").find('.mx-tab');
			var allFrame = tab.find('.arrowFrame');
			var allArrow = tab.find('i');
	        
			//$('body').not(".mx-tab").click(function () {
			...
			tab.each(function(i, item) {
			    $(item).click(function() {
			        var oframe = $(this).find('.arrowFrame');
		...
		$(function() {
			...
			function LoadList(flag) {
				...
				jQuery.ajax({
					...
					data: {
						Cmd: "GetMyFtbInvestList",
						pageIndex: pageIndex,
						weOrderId: "<%= Request.QueryString["OrderId"]%>"
					},
					dataType: "json",
						...
						if(jsonstr.result == "1") {
							$(jsonstr.msg).each(function(index, item) {
								...
								if(item.IsHaveTranList) {
									contracthtml = "<div class='rf mx-tab'>" +
										"<i></i>" +
										"承接明细" +
										"<div class='arrowFrame' style='display: none;'>" +
