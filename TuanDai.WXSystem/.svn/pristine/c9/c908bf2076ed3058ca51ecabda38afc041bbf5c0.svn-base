(function($){
    $.fn.yxMobileSlider = function(settings){
        ...
        return this.each(function(){
            ...
            var num =  $(this).find('li').length; //滚动屏幕数
            var oPosition = {}; //触点位置
            var moveWidth = s.width; //滚动宽度
            var focus_con = s.focus_con || _this;
            //初始化主体样式
            ...
            //初始化焦点容器及按钮
            focus_con.append('<div class="focus"><div></div></div>');
            var oFocusContainer = focus_con.find('.focus');
            for (var i = 0; i < num; i++) {
            ...
            function doAnimate(iTarget, fn){
                    ...
                    $(".curve").hide().css('opacity', '0');
                    $(".fk_model_r_tab3  .curve").hide().css('opacity', '1');
                    $(".fk_model_r span i").stop().css("height","0");
                oMover.stop().animate({
                    left: iTarget
                }, _this.speed , function(){
                    if (fn)fn();

                    var curIndex = Math.abs(iTarget/moveWidth)+1;

        
                     for(var k=0;k<$(".fk_model_r_tab"+curIndex).find("span").length;k++)
                     {
                    $(".fk_model_r_tab"+curIndex).find("span").eq(k).find("i").stop().animate({
                        height:"100%"
                    },
                    800,function(){$(".fk_model_r_tab"+curIndex+"  .curve").show().stop().css("opacity", "0").animate({
                        opacity: 1
                    },
                    1000);
                });
