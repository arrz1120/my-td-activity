...
function LoadProjectList(flag) {
    ...
    var paramObj = { Cmd: "GetZQZRProjectShowList", queryType: queryType, orderByDesc: orderByDesc,
...
function getFloatDivideStr(rate,flag){
    if(flag==1)
        ...
        if(parseFloat(rate)%1==0)
            ...
            var tmp= parseFloat(rate).toFixed("f2");
            return "."+ tmp.substr(tmp.lastIndexOf(".")+1,2);
        }
    }
}
