...
function RegSubmit(ctrl) {
    if (BlurTelNo(true) && ValidateImgCode() && BlurCode(true)) {
        ...
        $.ajax({
            ...
            type: "post",
            data: {
                cmd: "RegisterUser", mobilenumber: $("#phone").val(), verificationCode: $("#messCode").val(), ExtendKey: extendKey, from: GetQueryString("tdfrom") == null ? GetQueryString("from") : GetQueryString("tdfrom")
            },
            success: function (json) {
                AsyncReg(json);
            },
...
function AsyncReg(data) {
    if (data.result == "1") {
        window.location.href = "result.aspx?tel=" + $("#phone").val();
    }
    else {
        $("#btnReg").html("马上领取");
        $("#loginWrap").show();
