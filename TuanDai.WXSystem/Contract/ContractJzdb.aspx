<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ContractJzdb.aspx.cs" Inherits="TuanDai.WXApiWeb.Contract.ContractJzdb" %>
<!doctype html>
<html>
<head>
    <meta charset="utf-8">
...
<body>
    ...
    <div class="Contract">
        ...
        <div class="Contract_c">
               ...
               <p> 一、借款条款<br />
                ...
                2、借款期限：<span>*</span>个月，自甲方出借之日（甲方在居间方平台投资乙方发布的借款标之日）起计算。<br />
                ...
                6、本次借款期间，乙方有权提前一次性结清本次债务。若乙方选择等额本息的还款方式，利息应按照实际借款期间足月计算(即不足一个月的按一个月计算)。 若乙方选择每月付息且到期本金的还款方式，在乙方提前结清本次借款时侯，应向甲方支付实际已产生的利息（即按照实际用款天数折比365个自然日计算）。
               </p>
              <p>
                二、债的担保<br />
                乙方在居间方平台上有已出借且待收回的本息款项（以下简称“待收款项），乙方授权居间方在其待收款项回款后将回款资金中的￥<span><%=PrincipalInterest.ToString("N") %></span>元（大写：人民币<span><%=Tool.ChineseNum.GetUpperMoney(double.Parse(PrincipalInterest.ToString())) %></span>）划转至甲方在居间方平台开立的账户（以下简称”甲方平台账户“），若待收款项回款日期早于本借款合同约定的借款期限，乙方同意居间方对该回款资金进行冻结，乙方在本合同借款期限届满前不得提现，并于借款期限届满之日将￥<span><%=PrincipalInterest.ToString("N") %></span>元（大写：<span><%=Tool.ChineseNum.GetUpperMoney(double.Parse(PrincipalInterest.ToString())) %></span>）自动划转至甲方平台账户。若乙方因本合同借款出现逾期而产生逾期利息或违约金的，逾期利息与违约金一并由代收款项中扣除，代收款项不足以支付的，居间方自使用其友情垫付应急基金（以下简称“应急基金”)垫付后有权向乙方进行追讨。
              </p>
        </div>
    </div>
    <!--第2页>-->
    <div class="Contract">
        <div class="Contract_c">
            ...
            <p>2、本金逾期超过30天情形</p>
            <p class="text-ind">（1）居间方会根据甲方投资时的会员等级，在乙方本金逾期的第31日使用其应急基金进行流动性垫付：若甲方为超级会员，则垫付甲方应收未收的本息、逾期利息和违约金；若甲方为普通会员，则垫付甲方应收未收的本金；自居间方使用其应急基金进行垫付后，乙方对甲方（普通会员）借款本金自逾期之日产生的逾期利息与违约金归居间方应急基金所有，居间方有权向乙方进行追讨。</p>
