	...
	Jsbridge.prototype = {
		...
		*/
		...
		toAppP2p: function() {
			try {
				if (isIOS()) {
					typeof ToAppP2p === 'function' && ToAppP2p();
				} else {
					androidHandler('ToAppP2p', null);
				}
			} catch (e) {
				console.info("不支持jsbridge", e);
			}
		...
		toAppRegularFi: function() {
			try {
				if (isIOS()) {
					typeof ToAppRegular_fi === 'function' && ToAppRegular_fi();
				} else {
					androidHandler('ToAppRegular_fi', null);
				}
			} catch (e) {
				console.info("不支持jsbridge", e);
			}
		...
		toAppCallService: function() {
			try {
				if (isIOS()) {
					typeof ToAppIosCallService === 'function' && ToAppIosCallService();
				} else {
					androidHandler('ToAppCallService', null);
				}
			} catch (e) {
				console.info("不支持jsbridge", e);
			}
		...
		toAppOnlineService: function() {
			try {
				if (isIOS()) {
					typeof ToAppIosOnlineService === 'function' && ToAppIosOnlineService();
				} else {
					androidHandler('ToAppOnlineService', null);
				}
			} catch (e) {
				console.info("不支持jsbridge", e);
			}
