...
<html>
	...
	<body class="bg-f1f3f5">
	    <%= this.GetNavStr()%>
		<div id="bigDiv">
			<a href="javascript:void(0);" class="data-tips">因数据同步问题，新产生的回款及结清记录存在延时情况</a>
            ...
            <div class="bb-e6e6e6 bt-e6e6e6 pl15 bg-fff mt10">
                ...
                <div class="pt15 pb8" id="dvReturn">
					...
					<%--<div class="sam-p2 clearfix">
						...
						<div class="rf w50p c-ababab f12px" id="dueRecShouYi">预期收益：0.00</div>
					</div>--%>
				</div>
                <% if (GlobalUtils.IsShowZxToBorrower(WebUserAuth.UserId.Value))
                   {
                       %>
                <div class="pt15 pb8 pos-r" id="dvJieQing">
                	<div class="zx-item">智享转让</div>
                ...
                <div class="pt15 pb8 pos-r" id="dvRepay">
                	<div class="zcb-item">资产标</div>
					<p class="sam-p1 f18px"><i class="sam-icon sam-blue"></i>待结清<span class="c-fd6040 f18px ml15" id="dueOutTotal">0.00</span></p>
					<%--<div class="sam-p2 clearfix">
		    ...
		    function LoadData(actIndex) {
		        ...
		        $.ajax({
		            ...
		            success: function (json) {
		                ...
		                //        $("#dueRecShouYi").html("预期收益：" + fmoney2(parseFloat(json.dueRecShouYi) + parseFloat(json.dueRecJiangLi)));
		                ...
		                if (data != null && data.length > 0) {
		                    MonthLi.each(function (i, li) {
		                        ...
		                        $(data).each(function (index, item) {
		                            if (parseFloat(item.payMoney) > 0 || parseFloat(item.payZxMoney) > 0 || parseFloat(item.returnMoney) > 0) {
		                                if (parseInt(item.myDate.substr(item.myDate.length -2 ,2)) == parseInt($(li).html())) {
		                                    $(li).addClass('red').attr({ 'returnMoney': item.returnMoney, 'repayMoney': item.payMoney, 'repayZxMoney': item.payZxMoney, 'date': item.myDate });
		                                    AddTips($(li), item.returnMoney, item.payMoney, item.payZxMoney);
		                                }
		                            }
		                        });
		                    });
		                    $(data).each(function (index, item) {
		                        if (parseFloat(item.payMoney) > 0 || parseFloat(item.payZxMoney) > 0 || parseFloat(item.returnMoney) > 0) {
		                            if (parseFloat(item.payMoney) > 0) {
		                                payMoney += parseFloat(item.payMoney);
		                            }
		                            if (parseFloat(item.payZxMoney) > 0) {
		                                payZxMoney += parseFloat(item.payZxMoney);
		                            }
		                            if (parseFloat(item.returnMoney) > 0) {
		                                returnMoney += parseFloat(item.returnMoney);
		                            }
		                        }
		                    });
		                } else {
		                ...
		                if ((parseFloat(returnMoney)) <= 0) {
		                    ...
		                    $("#dueRecTotal").html(fmoney2(parseFloat(returnMoney)));
		                        //$("#dueRecBenJin").html("本金：" + fmoney2(retBen));
		                        //$("#dueRecShouYi").html("预期收益：" + fmoney2(retLi));
		                ...
		                if ((parseFloat(payMoney)) <= 0) {
		                    ...
		                    $("#dueOutTotal").html(fmoney1(parseFloat(payMoney)));
		                        //$("#dueOutBenJin").html("本金：" + fmoney2(payBen));
		                        //$("#dueOutLiXi").html("利息：" + fmoney1(payLi));
		                        $("#dvRepay").show();
		                    }

		                if ((parseFloat(payZxMoney)) <= 0) {
		                        $("#dvJieQing").hide();
		                    } else {
		                    $("#dueOutZxTotal").html(fmoney1(parseFloat(payZxMoney)));
		                        //$("#dueOutZxBenJin").html("本金：" + fmoney2(payZxBen));
		                        //$("#dueOutZxLiXi").html("利息：" + fmoney1(payZxLi));
