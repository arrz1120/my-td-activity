<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="return_pay_list.aspx.cs" Inherits="TuanDai.WXApiWeb.Member.Repayment.return_pay_list" %>
...
<html>
	...
	<body>
		...
		<div class="float-jump" style="bottom: 57px;">
			<a href="/Index.aspx">
				<img src="/imgs/images/icon/ico_toHome.png"></a>
			<a href="/Member/my_account.aspx">
				<img src="/imgs/images/icon/ico_toAccount.png"></a>
		</div>
		<div class="rmWidth rmNav clearfix">
			<div class="w1 lf pl20rem">
				<p class="f30rem">日期</p>
			</div>
			<div class="w2 lf text-right">
				<p class="f30rem">待回款</p>
			</div>
            <% if (GlobalUtils.IsShowZxToBorrower(WebUserAuth.UserId.Value))
               {
                   %>
            <div class="w3 lf text-right">
				<p class="f30rem">待结清</p>
				<p class="f24rem c-808080 nav-pt">（智享转让）</p>
			</div>
            <%
               } %>
			
			<div class="w5 lf text-right">
				<p class="f30rem">待结清</p>
				<p class="f24rem c-808080 nav-pt">（资产标）</p>
			</div>
		</div>
			...
			<div class="d_tab clearfix bg-fff">
                ...
                <% if (GlobalUtils.IsShowZxToBorrower(WebUserAuth.UserId.Value))
               {
				...
				<div class="lf w33p text-center pt8 pb10" id="tab2">
					<p class="f15px">待结清</p>
                    <p class="f24rem nav-pt">（资产标）</p>
					<%--<p class="f13px line-h18 c-808080" id="dueOutAmount">3000.00</p>--%>
				</div>
			</div>
			<!--待回款-->
			<div id="list1">
		...
		<script type="text/javascript">
		    var isShowZxOut = "<%=GlobalUtils.IsShowZxToBorrower(WebUserAuth.UserId.Value)%>";
		    //点击月份事件
		                ...
		                success: function(jsonstr) {
		                    ...
		                    if(jsonstr.length > 0) {
		                        ...
		                        $(jsonstr).each(function (ind, rowItem) {
		                            var zxOut = "";
		                            if (isShowZxOut == "True") {
		                                zxOut = '<div class="w5 lf f30rem text-overflow text-right">' + fmoney1(rowItem.payZxMoney) + '</div>';
		                            }
		                            if(parseFloat(rowItem.returnMoney) > 0 || parseFloat(rowItem.payMoney) > 0 || parseFloat(rowItem.payZxMoney) > 0) {
		                                row = '<div onclick="javascript:openAlert($(this));" class="rmWidth rmItem dayItem clearfix" day="' + rowItem.myDate.substr(8, 2) + '" date="' + rowItem.myDate.substr(0, 10) + '" returnMoney="' + fmoney2(rowItem.returnMoney) + '" repayMoney="' + (parseFloat(rowItem.payMoney) > 0 ? fmoney2(parseFloat(rowItem.payMoney) + 0.01) : 0.00) + '" repayZxMoney="' + fmoney1(rowItem.payZxMoney) + '">' +
		                                    ...
		                                    '<div class="w2 lf f30rem text-overflow text-right">' + fmoney2(rowItem.returnMoney) + '</div>' +
		                                    zxOut+
		                                        '<div class="w3 lf f30rem text-overflow text-right">' + fmoney1(rowItem.payMoney) + '</div>' +
		            ...
		            success: function(jsonstr) {
		                ...
		                if(jsonstr.list && jsonstr.list.length > 0) {
		                    ...
		                    $(jsonstr.list).each(function(index, item) {
		                        if(parseFloat(item.returnMoney) > 0 || parseFloat(item.payMoney) > 0 || parseFloat(item.payZxMoney) > 0) {
		                            ...
		                            var rowZxOut = "";
		                            if (isShowZxOut == "True") {
		                                rowZxOut = '<div class="w5 lf f30rem text-overflow text-right">' + fmoney1(item.payZxMoney) + '</div>';
		                            }
		                            row = yearStr + '<div class="rmWidth rmItem monthItem clearfix" onclick="javascript:clickMonth(this);" year="' + item.myMonth.substr(0, 4) + '" month="' + item.myMonth.substr(5, 2) + '">' +
		                                ...
		                                '<div class="w2 lf f30rem text-overflow text-right">' + fmoney2(item.returnMoney) + '</div>' +
		                                //isShowZxOut == "True"?'<div class="w5 lf f30rem text-overflow text-right">' + fmoney1(item.payZxMoney) + '</div>':"" +
		                                rowZxOut +
		                                '<div class="w3 lf f30rem text-overflow text-right">' + fmoney1(item.payMoney) + '</div>' +
		    ...
		    function LoadDetail(li) {
		        ...
		        $.ajax({
		            ...
		            success: function(json) {
		                ...
		                $("#dueOutAmount").html(fmoney2(repayMoney));
		                $("#dueOutZxAmount").html(fmoney2(repayZxMoney));
		                if(json.length > 0) {
		                    $("#d1").html("");
		                    $("#d2").html("");
		                    $("#d3").html("");
