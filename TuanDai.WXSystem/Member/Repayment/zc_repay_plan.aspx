<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="zc_repay_plan.aspx.cs" Inherits="TuanDai.WXApiWeb.Member.Repayment.zc_repay_plan" %>
		                    ...
		                    if (jsonstr.length > 0) {
		                        ...
		                        $(jsonstr).each(function (ind, rowItem) {
		                            if (parseFloat(rowItem.payMoney) > 0) {
		                                row = '<div onclick="javascript:openAlert($(this));" class="rmWidth rmItem dayItem clearfix" day="' + rowItem.myDate.substr(8, 2) + '" date="' + rowItem.myDate.substr(0, 10) + '" returnMoney="' + fmoney2(rowItem.returnMoney) + '" repayMoney="' + fmoney1(rowItem.payMoney) + '">' +
		            ...
		            success: function (json) {
		                ...
		                if (json.length > 0) {
		                    ...
		                    $(json).each(function (index, item) {
		                        if (parseFloat(item.Amount) + parseFloat(item.InterestAmout) + parseFloat(item.JiangLi) > 0 && item.IsReturn != "1" && item.Type != "37" && item.Type != "38") {
