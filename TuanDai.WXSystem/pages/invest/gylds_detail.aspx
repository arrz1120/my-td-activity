<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="gylds_detail.aspx.cs" Inherits="TuanDai.WXApiWeb.pages.invest.gylds_detail" %>
...
<div class="dropContainer"> 
			...
			<div class="dropdownCon hide">
				<div class="c-808080 f14px pb15 bb-e6e6e6 text-justify pr15">  
                 <p class="pb15 c-808080 f14px">
                    企业简介：<%=model.BusinessDesc %>
                </p>
                <p class="pb15 c-808080 f14px pt15" >审核借款额度：俊拓金融通过审核、分析借款企业历史各项真实经营数据(销售额、订单数量等)，确定借款企业的借款额度为
                     <%= decimal.Parse(this.borrowerUserInfo.CreditGrantingAmount.ToString()) >= 10000 ? ToolStatus.ConvertWanMoney(this.borrowerUserInfo.CreditGrantingAmount) + "万" : ToolStatus.ConvertLowerMoney(this.borrowerUserInfo.CreditGrantingAmount).Replace(".00","") %>元，本次借款                   
                     <%= decimal.Parse(model.Amount.ToString()) >= 10000 ? ToolStatus.ConvertWanMoney(model.Amount) + "万" : ToolStatus.ConvertLowerMoney(model.Amount).Replace(".00","") %>元。
                </p>
                <p class="pb15 c-808080 f14px pt15">借款用途：<%=model.AmountUsedDesc %></p>
				</div>
			</div>
		</div>
		...
		<div class="bt-e6e6e6 bb-e6e6e6 bg-fff mt15 pl15">
			...
			<div class="dropdownCon hide">
				<div class="pb15 c-808080 f14px text-justify pr15 span_txt pos-r"><span class="f14px c-808080">•</span>企业经营收入、应收账款回收是本项目主要还款来源；</div>
				<div class="pb15 c-808080 f14px text-justify pr15 span_txt pos-r"><span class="f14px c-808080">•</span>项目担保方<%=DanBaoCompany%>连带责任担保。</div>
			</div>
		</div>
       ...
       <% if(imageList!=null && imageList.Count>0){ %>
		...
		<div class="bt-e6e6e6 bb-e6e6e6 bg-fff mt15">
			<div class="pt10 pb10 c-212121 f17px pl15 dropdownTit"><i class="ico-sprite01 ico-triangle rotate0"></i>供应链产品概述</div>
			<div class="dropdownCon hide">
				<div class="pl20 pt18 pb25 c-808080 f14px pos-r">
					 供应链电商借款业务是团贷网联合深圳前海俊拓金融服务有限公司为淘宝网、维品会、京东、amazon等平台商家提供的一种无抵押信用借款。受制于应收账款回款的及时性，电商企业流动性资金一般较为紧张，如遇重大促销活动可能出现无采购资金可用的现象，企业扩大经营备受制约。通过电商企业店铺账号授权，以及生意参谋等第三方工具协助，俊拓金融准确获取商家真实交易数据，严谨评估周期应收账款，并利用大数据风控模式对商家进行合理确认借款额度；同时配合团贷网严审各项借款资料，一起助力优质电商企业快速借款、扩大经营。			 
			    </div>
			</div>
		</div> 
