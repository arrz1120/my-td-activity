...
monthSwiper.prototype = {
	...
	//添加回款借款提示
	addTips:function(obj,returnMoney,repayMoney,repayZxMoney){
	...
	lightDay:function(data){
		...
		for(var i=0;i<data.length;i++){
			...
			for(var j=0;j<data[i].length;j++){
				...
				li.each(function(m){
					if(li.eq(m).html()==day){
						li.eq(m).addClass('red').attr({
							'returnMoney':data[i][j].returnMoney,
							'repayMoney':data[i][j].repayMoney,
							'repayZxMoney':data[i][j].repayZxMoney
						});
						_this.addTips(li.eq(m),data[i][j].returnMoney,data[i][j].repayMoney,data[i][j].repayZxMoney);
					}
				})
