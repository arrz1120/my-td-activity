...
function getShowRedPacketList() {
    ...
    $.each(RedPacketObj.PrizeList, function (i, item) {
        ...
        redpacketHtml += '<div class="ticket-con {0} pos-r bg-fff" ' + attrHtml + ' >' +
                             ' <div class="ticket-d-wrap rp-pt">' +
                             '     <div class="tx-ticket-details pos-r">' +
        ...
        if (item.IsAllowUsed) {
            if (item.IsCalcNoUsed == 0) {
                redpacketHtml = redpacketHtml.replace("{0}","red"); 
                redpacketHtml += '	</div>' +
                                '</div>' +(curPrizeId == item.PrizeId ? '<span class="icon-used"></span>' : "") +
                                '</div>';
            } else {
                redpacketHtml = redpacketHtml.replace("{0}", "gray");
                redpacketHtml +='		<p class="no-use pos-r"><span>不可用原因：</span>' + item.NoUseDesc + '</p>' +
                                '	</div>' +
                                ' </div>';
            }
        } else {
            redpacketHtml = redpacketHtml.replace("{0}", "gray");
            var noUsehtml = ""; 
            $.each(item.NoUseDescList, function (i, desc) {
                noUsehtml += "<em>" + desc + "</em>";
            });
            //<em>单笔投资金额不满1000元</em><em>单笔投资金额不满1000元</em>
            redpacketHtml += '		<p class="no-use pos-r"><span>不可用原因：</span>' + noUsehtml + '</p>' +
                            '	</div>' +
                            ' </div>';
        }
        redpacketHtml += "</div>";
